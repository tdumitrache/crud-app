...
app.get("/meetings", (req, res) => {
  ...
  app.post("/add/students", (req, res) => {
    ...
    db.query(
        "INSERT INTO students(first_name, last_name, age, email, country, year_of_study) VALUES(?, ?, ?, ?, ?, ?)", [firstName, lastName, age, email, country, yearOfStudy],
        (err, result) => {
          if (err) {
            console.log(err);
          } else {
  ...
  app.post("/add/departments", (req, res) => {
    ...
    db.query(
        "INSERT INTO departments(name, max_students) VALUES(?, ?)", [name, maxStudents],
        (err, result) => {
          if (err) {
            console.log(err);
          } else {
  ...
  app.post("/add/meetings", (req, res) => {
    ...
    db.query(
        "INSERT INTO meetings(id_student, id_department, date, nr_of_students) VALUES(?, ?, ?, ?)", [idStudent, idDepartment, date, nrOfStudents],
        (err, result) => {
          if (err) {
            console.log(err);
          } else {
  ...
  app.delete("/delete/departments/:id", (req, res) => {
    const id = req.params.id;
    db.query("DELETE FROM departments WHERE id_department = ?", id, (err, result) => {
      if (err) {
        console.log(err);
      } else {
        res.send(result);
  ...
  app.put("/update/students/:id", (req, res) => {
    const { firstName, lastName, age, email, country, yearOfStudy, idStudent } = req.body;
